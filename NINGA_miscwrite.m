function NINGA_miscwrite(varargin)
% Write various scalar data formats based on the struct X.
% 
% NINGA_miscwrite(X);
% 
% See 'NINGA_miscread.m' for a description of the contents of X.
%
...
switch lower(X.readwith),
    ...
    case 'vestread',
        ...
        NINGA_vestwrite(X.filename,X.data);
        
    case 'msetread',
        
        % Write an MSET (matrix set) file
        NINGA_msetwrite(X.filename,X.data);
    ...
    case 'wb_command',
        ...
        NINGA_ciftiwrite(X.filename,X.data,X.extra,[],toscalar);
    ...
    case 'dpxread',
        ...
        NINGA_dpxwrite(X.filename,X.data,X.extra.crd,X.extra.idx);
